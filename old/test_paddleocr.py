"""
Тестовый скрипт для распознавания паспорта через PaddleOCR
Документация: https://github.com/PaddlePaddle/PaddleOCR
"""
...
def extract_passport_fields(ocr_results):
    ...
    PaddleOCR возвращает массив: [bbox, (text, confidence)]
    """
    all_text = []
...
def main():
    print("=" * 70)
    print("PaddleOCR - Тест распознавания паспорта РФ")
    print("=" * 70)
    print()

    # Инициализация PaddleOCR
    print("⏳ Инициализация PaddleOCR (первый запуск может занять время)...")
    try:
        # Минимальная конфигурация для совместимости с разными версиями PaddleOCR
        ocr = PaddleOCR(
            lang='ru',                         # Русский язык
            use_textline_orientation=True      # Автоматический поворот изображения
        )
        print("✅ PaddleOCR инициализирован успешно")
        print()
    except Exception as e:
        print(f"❌ Ошибка инициализации PaddleOCR: {e}")
        print("\nПроверьте установку:")
    ...
    while True:
        ...
        try:
            ...
            output_file = image_path.rsplit('.', 1)[0] + '_paddleocr_result.json'
            with open(output_file, 'w', encoding='utf-8') as f:
                json.dump({
                    'passport_data': passport_data,
                    'raw_ocr_result': [[bbox, [text, float(conf)]] for bbox, (text, conf) in result[0]]
